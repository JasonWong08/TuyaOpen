...
************************macro define************************
***********************************************************/
#ifndef OVERFLOW_PSRAM_STOP_TYPE
#define OVERFLOW_PSRAM_STOP_TYPE OVERFLOW_STOP_TYPE
#endif

/* On no-PSRAM targets, manual wakeup mode does not need long VAD cache.
 * Keep a small safety window to reduce heap pressure for prompt playback. */
#define AI_AUDIO_MANUAL_RINGBUF_MIN_MS 160
...
@return None
*/
static void __audio_frame_put(TDL_AUDIO_FRAME_FORMAT_E type, TDL_AUDIO_STATUS_E status, uint8_t *data,
                              uint32_t data_len)
{
    /* Microphone disabled? */
...
static void __record_task(void *arg)
{
    ...
    while (sg_recorder->vad_task && tal_thread_get_state(sg_recorder->vad_task) == THREAD_STATE_RUNNING) {
        ...
        if (sg_recorder->vad_mode == AI_AUDIO_VAD_AUTO) {
            AI_AUDIO_VAD_STATE_E stat =
                (tkl_vad_get_status() == TKL_VAD_STATUS_SPEECH) ? AI_AUDIO_VAD_START : AI_AUDIO_VAD_STOP;
            if (stat != sg_recorder->vad_flag) {
                PR_DEBUG("audio input -> wakup flag is %d, auto vad set from %d to %d!", sg_recorder->wakeup_flag,
...
static AI_AUDIO_RECODER_T *__audio_recorder_create(AI_AUDIO_INPUT_CFG_T *cfg, TDL_AUDIO_INFO_T *audio_info)
{
    ...
    uint32_t rb_cache_ms = cfg->vad_active_ms + 300;
    if (cfg->vad_mode == AI_AUDIO_VAD_MANUAL) {
        uint32_t manual_cache_ms = cfg->slice_ms * 2;
        if (manual_cache_ms < AI_AUDIO_MANUAL_RINGBUF_MIN_MS) {
            manual_cache_ms = AI_AUDIO_MANUAL_RINGBUF_MIN_MS;
        }
        rb_cache_ms = manual_cache_ms;
    }
    sg_recorder->vad_size = rb_cache_ms * audio_1ms_size + 1;
    ...
    PR_DEBUG("recorder total ms %d, cache ms %d, slice ms %d, vad active %d ms, vad off timeout %d", rb_size,
             rb_cache_ms, cfg->slice_ms, cfg->vad_active_ms, cfg->vad_off_ms);
