...
static void __second_uart_log_tx(const uint8_t *data, size_t length)
{
    ...
    PR_NOTICE("second_uart: TX UART%d GPIO%d/%d %u bytes: \"%.*s\"",
              (int)SECOND_UART_HW_PORT, (int)SECOND_UART_TX_PIN, (int)SECOND_UART_RX_PIN,
              (unsigned)length, (int)length, (const char *)data);
}
...
OPERATE_RET second_uart_init(void)
{
    ...
    s_initialized = true;
    s_last_len    = 0;
    PR_NOTICE("second_uart: ready UART%d TX=GPIO%d RX=GPIO%d @ %d baud",
              (int)SECOND_UART_HW_PORT, (int)SECOND_UART_TX_PIN, (int)SECOND_UART_RX_PIN,
              SECOND_UART_BAUD_RATE);
    return OPRT_OK;
}
