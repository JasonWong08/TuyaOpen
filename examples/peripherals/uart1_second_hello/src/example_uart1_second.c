 ...
 * USB debug log uses UART0 (GPIO43/44); do not confuse it with the 4-pin extension port.
 */
...
void user_main(void)
{
    ...
    PR_NOTICE("UART1 TX=GPIO%d  RX=GPIO%d  baud=%d", (int)UART1_TEST_TX_PIN, (int)UART1_TEST_RX_PIN, UART1_TEST_BAUD);
    ...
    PR_NOTICE("UART1 init OK. Sending \"%s\" every %d ms", "hello, world.", UART1_HELLO_INTERVAL_MS);

    while (1) {
        int written = tal_uart_write(UART1_TEST_PORT, (const uint8_t *)UART1_HELLO_LINE, strlen(UART1_HELLO_LINE));
